...
public class ClassController extends ADiagramEditorController
     ...
     */
    protected final IClassDiagram diagram;

    /**
     * The menu item that will allow the creation.
     */
    protected MenuItem createClassMI;
    protected MenuItem createInterfaceMI;
    protected MenuItem createEnumMI;
    protected MenuItem createAbstractClass;

    /**
     * The context menu that will contain the menu item.
     */
    protected ContextMenu ctxMenuDiagram;

    /**
     * The x, y position of mice.
     */
    protected double miceX;
    protected double miceY;

    /**
     * A map that will contain all note, the model of note as key and the graphic as value.
     */
    protected final Map<INoteClass, NoteGraphic> notes;

    /**
     * A map that will contain all entity, the graphic model of entity as key and graphic as value.
     */
    protected final Map<IViewEntity, ObjectEntityGraphic> viewEntities;

    /**
     * A map that contain the models of an entity, the intern model as key, the graphic model as value.
     */
    protected final Map<IObjectEntity, IViewEntity> objectEntities;

    /**
     * A map that will contain all relation, the model as key and graphic as value.
     */
    protected final Map<IViewRelation, IRelationGraphic> viewRelations;

    /**
     * An object property for the creation of an note about relation.
     */
    protected final ObjectProperty<INoteClass> createNoteRelation;

    /**
     * An object property for the creation of a relation.
     */
    protected final ObjectProperty<IViewEntity> createEntityRelation;

    /**
     * The note's relation handler.
     */
    protected EventHandler<MouseEvent> addNoteRelationEvt;

    /**
     * The entity's relation handler.
     */
    protected EventHandler<MouseEvent> addEntityRelationEvt;

    /**
     * A map that will contain all package, the model of package as key and the graphic as value.
     */
    protected final Map<IViewEntity, PackageGraphic> packages;

    /**
     * An object property for the creation of a relation.
     */
    protected final ObjectProperty<IViewEntity> createPackageRelation;
    ...
    @Override
    public Map<IObjectEntity, IViewEntity> getObjectEntities() {
     ...
     * @return INote
     */
    protected INoteClass getNote(INoteGraphic noteG) {
    ...
    public void createEntity(double x, double y, TypeEntity t) {
        ...
        if (entity != null) {
        	if (getDiagram().getEntities().size() < 10) {
	     		int pos = 10 + 35 * getDiagram().getEntities().size();
	            entity.addStyle(IDiagramEditorController.POSITION_STYLE_ID, x + "|" + y + "|" + pos + "|50");
        	} else {
	            entity.addStyle(IDiagramEditorController.POSITION_STYLE_ID, x + "|" + y + "|150|140");
        	}
        }
    }
    ...
    public void createNote(double x, double y) {
        ...
        if (note != null) {
        	int pos = 10 + 60 * getDiagram().getNotes().size();
            note.addStyle(POSITION_STYLE_ID, x + "|" + y + "|" + pos + "|300");
        }
    }
                ...
                addEntityRelationEvt = new EventHandler<MouseEvent>() {
                    ...
                    public void handle(MouseEvent event) {
                        if (event.getSource().getClass() == NoteGraphic.class) {
                        	...
                        	PackageGraphic oeg = (PackageGraphic) event.getSource();
                            IRelation r = new Relation(
                                    Math.random() + "", newValue.getData(),
                                    oeg.getViewEntity().getData(),
                                    TypeHeadArrow.None,
     ...
     */
    protected void createEntityGraphic(IViewEntity entity) {
     ...
     */
    protected void createNoteGraphic(INoteClass note) {
     ...
     */
    protected void createRelationGraphic(IViewRelation vr) {
    	try {
        	IEntityGraphic src = viewEntities.get(objectEntities.get(vr.getEntitySource()));
        	if (src == null) {
        		src = packages.get(objectEntities.get(vr.getEntitySource()));
        	}
        	IEntityGraphic dst = viewEntities.get(objectEntities.get(vr.getEntityTarget()));
        	if (dst == null) {
        		dst = packages.get(objectEntities.get(vr.getEntityTarget()));
        	}
	...
	@Override
	public void removeNote(INote modelNote) {
		// Voir removeNote(INoteClass) inutile
	}

	@Override
	public void createRelation(IEntityRelation entity) {
		// Voir createEntityRelation(IViewEntity) inutile
	}

	@Override
	public void removeRelation(fr.univrouen.umlreverse.model.diagram.common.IRelation relationModel) {
		// Voir removeRelation(IViewRelation) inutile
	}
}
