...
// ATTRIBUTES
		...
		addPointMI.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (result == ButtonType.YES) {
					...
					((ARelationGraphic) relation).addPointInRelation2(xPoint, yPoint);
     ...
     */
    private void edit() {
    	/*if (diagramController instanceof PackageController) {
    		PackageController packageController = (PackageController) diagramController;
    		if (packageController.getObjectEntities()
    					.get(viewRelation.getEntitySource()) instanceof IViewPackage
						|| packageController.getObjectEntities()
							.get(viewRelation.getEntitySource()) instanceof IViewPackage) {
